...
const pendingTicket = () => {
  ...
  const columns = [
    { id: 'ticketNumber', label: 'Subject', minWidth: 170 },
        ...
        const transformedRows = response?.data?.tickets.map((tickets, index) => ({
          ...
          priority: (<>
            <Grid sx={{ width: "100px", marginLeft: 5 }}>
              <Typography className={`${tickets?.priority === null ? `${classes.high}` : `${classes.enable}`}`}> {tickets?.priority === null ? 'N-I-R' : tickets?.priority}</Typography>
            </Grid></>),
          dateTime: (<><Grid><Typography>{tickets?.createdAt?.dateTime}</Typography>
          </Grid></>),
          ticketNumber: (<><Grid><Typography sx={{ color: '#4634ff' }}><b>{tickets?.ticketNumber}</b></Typography></Grid></>),
          userEmail: (<><Grid><Typography sx={{ color: '#4634ff' }}><b>{tickets?.userEmail}</b></Typography></Grid></>),
          status: <Grid sx={{ width: "100px", marginLeft: 5 }}>
            <Typography className={`${tickets?.status === 'PENDING' ? `${classes.enable}` : `${classes.green}`}`}> {tickets?.status}</Typography>
          </Grid>,
          action: (
